...
#sed -i 's/		pkill /		\/bin\/bash \/temp\/random-ovpn-fix.sh \&\& pkill /g' "$bhf"
#sed -i 's/		eval /		\/bin\/bash \/temp\/random-ovpn-fix.sh \&\& eval /g' "$bhf"

bhf='/root/getvpnport.sh'
echo '[info] Fix '"$bhf"
# When error from getvpn port, pick random ovpn
sed -i 's/		kill -2 /		\/bin\/bash \/temp\/random-ovpn-fix.sh \&\& kill -2 /g' "$bhf"
